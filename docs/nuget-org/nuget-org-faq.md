...
Pozwala na to, że podjęto próbę zmiany konto Microsoft logowania z `account1@outlook.com` dla użytkownika NuGet.org z nazwą użytkownika `MyNuGetAccount1` na inną konto Microsoft za pomocą poczty e-mail `account2@outlook.com` . Zobaczysz błąd powyżej.

**Co oznacza błąd powyżej?**

Oznacza to, że istnieje inne konto NuGet.org, które jest skojarzone z konto Microsoftem, które próbujesz zmienić. w powyższym przykładzie konto Microsoft z wiadomościami e-mail `<account2@outlook.com>` są skojarzone z innym kontem NuGet.org z nazwą użytkownika `MyNuGetAccount2` .
...
1. Po wykonaniu tej czynności druga konto Microsoft z wiadomościami e-mail `account2@outlook.com` będzie bezpłatna do Twojego pierwszego konta NuGet.org `MyNuGetAccount1` . Wykonaj te same kroki, aby zmienić nazwy logowania firmy Microsoft na drugą konto Microsoft.
...
Jeśli podjęto próbę zalogowania się przy użyciu konto Microsoft, powiedzmy, używając poczty e-mail `account1@outlook.com` i zobaczysz błąd podobny do poniższego:
> _Konto z wiadomością e-mail " account1@outlook.com " jest połączone z innym kontem Microsoft._
>
> _Jeśli chcesz zaktualizować połączone konto Microsoft, możesz to zrobić na stronie Ustawienia konta._

**Co oznacza błąd powyżej?**

Po utworzeniu konta w usłudze NuGet.org istnieje adres e-mail komunikacji skojarzony z tym kontem. Jest to zazwyczaj takie samo, jak adres e-mail używany dla skojarzonych konto Microsoft. Można jednak wybrać inny adres e-mail na potrzeby komunikacji. Dlatego technicznie możesz mieć różne konto Microsoft, z `account2@outlook.com` którymi jest połączony z kontem NuGet.org z adresem e-mail komunikacji `account1@outlook.com` .
...
Aby ustalić, który konto Microsoft jest połączony z kontem NuGet.org przy użyciu adresu e-mail, należy użyć przepływu [pomocy przy logowaniu](#which-microsoft-account-is-linked-to-my-nugetorg-account) `account1@outlook.com` .
...
5. Przejdź do witryny [https://www.nuget.org/account/transform](https://www.nuget.org/account/transform). Umożliwi to konwertowanie konta NuGet.org na organizacji bez łączenia się z konto Microsoft.
...
Twoje konto prawdopodobnie zostało wcześniej zarejestrowane jako osobiste konto Microsoft i działało prawidłowo, ale teraz prawdopodobnie konto zostało zarejestrowane jako dzierżawa "niezarządzana" w Azure Active Directory (usługa tożsamości używana do uwierzytelniania kont Microsoft). 

Mogło to nastąpić, jeśli ty lub ktoś z organizacji (z @yourdomain.com adresem e-mail) zarejestrowanym w jednej z zintegrowanych usług AAD lub zarejestrowano samoobsługowy [Azure Active Directory](/azure/active-directory/users-groups-roles/directory-self-service-signup), który tworzy taką niezarządzaną dzierżawę dla używanej domeny konto Microsoft ( @yourdomain.com w Twoim przypadku). 
...
Jeśli chcesz zalogować się do NuGet.org przy użyciu konta konto Microsoft użytkownika ( @yourdomain.com ), użytkownik (lub administrator w firmie) będzie musiał przejąć prawo własności do usługi AAD przez wykonanie walidacji DNS w celu uwierzytelnienia użytkowników przy użyciu adresu e-mail " @yourdomain.com ". Postępuj zgodnie z instrukcjami dla [domen przejęcia przez administratora](/azure/active-directory/users-groups-roles/domains-admin-takeover) przez usługę Azure Active Directory. Po wykonaniu tej czynności normalne logowanie powinno zacząć działać.

**Nie chcę nic robić, co jest innym sposobem na odzyskanie mojego konta?**

Można [utworzyć](https://www.microsoft.com/account) nowy konto Microsoft (z adresem e-mail, z którym **nie** jest skojarzona @yourdomain.com ). Wykonaj czynności podane w sekcji [Odzyskiwanie konta NuGet.org](#unable-to-use-microsoft-login-how-do-i-recover-my-nugetorg-account) .
