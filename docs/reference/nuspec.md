...
## <a name="project-type-compatibility"></a><span data-ttu-id="2e3f5-116">Zgodność typu projektu</span><span class="sxs-lookup"><span data-stu-id="2e3f5-116">Project type compatibility</span></span>
...
<span data-ttu-id="2e3f5-126">W tym schemacie `.nuspec` plik ma następującą formę ogólną:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-126">Within this schema, a `.nuspec` file has the following general form:</span></span>
...
<span data-ttu-id="2e3f5-136">Te elementy muszą znajdować się w obrębie `<metadata>` elementu.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-136">These elements must appear within a `<metadata>` element.</span></span>
...
<span data-ttu-id="2e3f5-151">Podczas przekazywania pakietu do nuget.org, `authors` pole jest ograniczone do 4000 znaków.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-151">When uploading a package to nuget.org, the `authors` field is limited to 4000 characters.</span></span>
...
#### <a name="license"></a><span data-ttu-id="2e3f5-165">license (licencja)</span><span class="sxs-lookup"><span data-stu-id="2e3f5-165">license</span></span>
...
#### <a name="icon"></a><span data-ttu-id="2e3f5-183">ikona</span><span class="sxs-lookup"><span data-stu-id="2e3f5-183">icon</span></span>
...
#### <a name="requirelicenseacceptance"></a><span data-ttu-id="2e3f5-194">requireLicenseAcceptance</span><span class="sxs-lookup"><span data-stu-id="2e3f5-194">requireLicenseAcceptance</span></span>
...
<span data-ttu-id="2e3f5-208">Podczas przekazywania pakietu do nuget.org, `releaseNotes` pole jest ograniczone do 35 000 znaków.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-208">When uploading a package to nuget.org, the `releaseNotes` field is limited to 35,000 characters.</span></span>
...
<span data-ttu-id="2e3f5-226">Na przykład:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-226">For example:</span></span>
```xml
<?xml version="1.0"?>
<package xmlns="http://schemas.microsoft.com/packaging/2010/07/nuspec.xsd">
    <metadata>
...
#### <a name="frameworkassemblies"></a><span data-ttu-id="2e3f5-241">frameworkAssemblies</span><span class="sxs-lookup"><span data-stu-id="2e3f5-241">frameworkAssemblies</span></span>
...
### <a name="metadata-attributes"></a><span data-ttu-id="2e3f5-258">atrybuty metadanych</span><span class="sxs-lookup"><span data-stu-id="2e3f5-258">metadata attributes</span></span>
...
| <span data-ttu-id="2e3f5-276">Token</span><span class="sxs-lookup"><span data-stu-id="2e3f5-276">Token</span></span> | <span data-ttu-id="2e3f5-277">Źródło wartości</span><span class="sxs-lookup"><span data-stu-id="2e3f5-277">Value source</span></span> | <span data-ttu-id="2e3f5-278">Wartość</span><span class="sxs-lookup"><span data-stu-id="2e3f5-278">Value</span></span>
| --- | --- | ---
...
<span data-ttu-id="2e3f5-306">`<dependencies>`Element w obrębie `<metadata>` zawiera dowolną liczbę `<dependency>` elementów, które identyfikują inne pakiety, od których zależy pakiet najwyższego poziomu.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-306">The `<dependencies>` element within `<metadata>` contains any number of `<dependency>` elements that identify other packages upon which the top-level package depends.</span></span> <span data-ttu-id="2e3f5-307">Atrybuty dla każdej z nich `<dependency>` są następujące:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-307">The attributes for each `<dependency>` are as follows:</span></span>
...
| <span data-ttu-id="2e3f5-332">kompilacja</span><span class="sxs-lookup"><span data-stu-id="2e3f5-332">build</span></span> | <span data-ttu-id="2e3f5-333">Kompilacja (właściwości i elementy docelowe programu MSBuild)</span><span class="sxs-lookup"><span data-stu-id="2e3f5-333">build (MSBuild props and targets)</span></span> |
...
<span data-ttu-id="2e3f5-382">W poniższym przykładzie przedstawiono odwołanie do `System.Net` dla wszystkich platform docelowych i odwołania do `System.ServiceModel` tylko dla .NET Framework 4,0:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-382">The following example shows a reference to `System.Net` for all target frameworks, and a reference to `System.ServiceModel` for .NET Framework 4.0 only:</span></span>
...
<span data-ttu-id="2e3f5-410">**Zestaw bibliotek DLL przy użyciu symbolu wieloznacznego**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-410">**Set of DLLs using a wildcard**</span></span>
...
<span data-ttu-id="2e3f5-414">Pliki zawartości to niezmienne pliki, których pakiet musi zawierać w projekcie.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-414">Content files are immutable files that a package needs to include in a project.</span></span> <span data-ttu-id="2e3f5-415">Są niezmienne, nie są przeznaczone do modyfikacji przez projekt zużywający.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-415">Being immutable, they are not intended to be modified by the consuming project.</span></span> <span data-ttu-id="2e3f5-416">Przykładowe pliki zawartości obejmują:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-416">Example content files include:</span></span>
...
- <span data-ttu-id="2e3f5-420">Pliki konfiguracji pakietu, które muszą być zawarte w projekcie, ale nie muszą mieć żadnych zmian specyficznych dla projektu</span><span class="sxs-lookup"><span data-stu-id="2e3f5-420">Configuration files for the package that need to be included in the project but don't need any project-specific changes</span></span>

<span data-ttu-id="2e3f5-421">Pliki zawartości są zawarte w pakiecie przy użyciu `<files>` elementu, określając `content` folder w `target` atrybucie.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-421">Content files are included in a package using the `<files>` element, specifying the `content` folder in the `target` attribute.</span></span> <span data-ttu-id="2e3f5-422">Jednak takie pliki są ignorowane, gdy pakiet zostanie zainstalowany w projekcie przy użyciu PackageReference, który zamiast tego używa `<contentFiles>` elementu.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-422">However, such files are ignored when the package is installed in a project using PackageReference, which instead uses the `<contentFiles>` element.</span></span>

<span data-ttu-id="2e3f5-423">Aby zapewnić maksymalną zgodność z zużywanymi projektami, pakiet najlepiej określa pliki zawartości w obu elementach.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-423">For maximum compatibility with consuming projects, a package ideally specifies the content files in both elements.</span></span>

### <a name="using-the-files-element-for-content-files"></a><span data-ttu-id="2e3f5-424">Używanie elementu Files dla plików zawartości</span><span class="sxs-lookup"><span data-stu-id="2e3f5-424">Using the files element for content files</span></span>

<span data-ttu-id="2e3f5-425">W przypadku plików zawartości wystarczy użyć tego samego formatu co w przypadku plików zestawu, ale określić `content` jako folder podstawowy w `target` atrybucie, jak pokazano w poniższych przykładach.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-425">For content files, simply use the same format as for assembly files, but specify `content` as the base folder in the `target` attribute as shown in the following examples.</span></span>

<span data-ttu-id="2e3f5-426">**Podstawowe pliki zawartości**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-426">**Basic content files**</span></span>
...
<span data-ttu-id="2e3f5-427">**Pliki zawartości z strukturą katalogów**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-427">**Content files with directory structure**</span></span>
...
<span data-ttu-id="2e3f5-428">**Plik zawartości specyficzny dla platformy docelowej**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-428">**Content file specific to a target framework**</span></span>
...
<span data-ttu-id="2e3f5-429">**Plik zawartości skopiowany do folderu z kropką w nazwie**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-429">**Content file copied to a folder with dot in name**</span></span>

<span data-ttu-id="2e3f5-430">W takim przypadku pakiet NuGet uważa, że rozszerzenie w programie nie jest `target` zgodne z rozszerzeniem w `src` i w ten sposób traktuje tę część nazwy w `target` postaci folderu:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-430">In this case, NuGet sees that the extension in `target` does not match the extension in `src` and thus treats that part of the name in `target` as a folder:</span></span>
...
<span data-ttu-id="2e3f5-431">**Pliki zawartości bez rozszerzeń**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-431">**Content files without extensions**</span></span>

<span data-ttu-id="2e3f5-432">Aby dołączyć pliki bez rozszerzenia, użyj `*` `**` symboli wieloznacznych lub:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-432">To include files without an extension, use the `*` or `**` wildcards:</span></span>
...
<span data-ttu-id="2e3f5-433">**Pliki zawartości ze szczegółową ścieżką i głębokiego celu**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-433">**Content files with deep path and deep target**</span></span>

<span data-ttu-id="2e3f5-434">W tym przypadku, ponieważ rozszerzenia plików dla dopasowania źródłowego i docelowego, pakiet NuGet zakłada, że obiektem docelowym jest nazwa pliku, a nie folder:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-434">In this case, because the file extensions of the source and target match, NuGet assumes that the target is a file name and not a folder:</span></span>
...
<span data-ttu-id="2e3f5-435">**Zmiana nazwy pliku zawartości w pakiecie**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-435">**Renaming a content file in the package**</span></span>
...
<span data-ttu-id="2e3f5-436">**Wykluczanie plików**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-436">**Excluding files**</span></span>
...
### <a name="using-the-contentfiles-element-for-content-files"></a><span data-ttu-id="2e3f5-437">Używanie elementu contentFiles dla plików zawartości</span><span class="sxs-lookup"><span data-stu-id="2e3f5-437">Using the contentFiles element for content files</span></span>

<span data-ttu-id="2e3f5-438">*Pakiet NuGet 4.0 + z PackageReference*</span><span class="sxs-lookup"><span data-stu-id="2e3f5-438">*NuGet 4.0+ with PackageReference*</span></span>

<span data-ttu-id="2e3f5-439">Domyślnie pakiet umieszcza zawartość w `contentFiles` folderze (patrz poniżej) i `nuget pack` uwzględnia wszystkie pliki w tym folderze przy użyciu atrybutów domyślnych.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-439">By default, a package places content in a `contentFiles` folder (see below) and `nuget pack` included all files in that folder using default attributes.</span></span> <span data-ttu-id="2e3f5-440">W takim przypadku nie trzeba umieszczać `contentFiles` węzła w ogóle `.nuspec` .</span><span class="sxs-lookup"><span data-stu-id="2e3f5-440">In this case it's not necessary to include a `contentFiles` node in the `.nuspec` at all.</span></span>

<span data-ttu-id="2e3f5-441">Aby kontrolować, które pliki są uwzględniane, `<contentFiles>` element Określa kolekcję `<files>` elementów, które identyfikują dokładne pliki include.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-441">To control which files are included, the `<contentFiles>` element specifies is a collection of `<files>` elements that identify the exact files include.</span></span>

<span data-ttu-id="2e3f5-442">Te pliki są określone za pomocą zestawu atrybutów, które opisują, jak powinny być używane w systemie projektu:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-442">These files are specified with a set of attributes that describe how they should be used within the project system:</span></span>

| <span data-ttu-id="2e3f5-443">Atrybut</span><span class="sxs-lookup"><span data-stu-id="2e3f5-443">Attribute</span></span> | <span data-ttu-id="2e3f5-444">Opis</span><span class="sxs-lookup"><span data-stu-id="2e3f5-444">Description</span></span> |
| --- | --- |
| <span data-ttu-id="2e3f5-445">**być**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-445">**include**</span></span> | <span data-ttu-id="2e3f5-446">Potrzeb Lokalizacja pliku lub plików do dołączenia, z uwzględnieniem wyjątków określonych przez `exclude` atrybut.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-446">(Required) The location of the file or files to include, subject to exclusions specified by the `exclude` attribute.</span></span> <span data-ttu-id="2e3f5-447">Ścieżka jest `contentFiles` określana względem folderu, chyba że określona jest ścieżka bezwzględna.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-447">The path is relative to the `contentFiles` folder unless an absolute path is specified.</span></span> <span data-ttu-id="2e3f5-448">Symbol wieloznaczny `*` jest dozwolony, a podwójne symbole wieloznaczne `**` oznacza cykliczne wyszukiwanie folderów.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-448">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="2e3f5-449">**wykluczanie**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-449">**exclude**</span></span> | <span data-ttu-id="2e3f5-450">Rozdzielana średnikami lista plików lub wzorców plików do wykluczenia z `src` lokalizacji.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-450">A semicolon-delimited list of files or file patterns to exclude from the `src` location.</span></span> <span data-ttu-id="2e3f5-451">Symbol wieloznaczny `*` jest dozwolony, a podwójne symbole wieloznaczne `**` oznacza cykliczne wyszukiwanie folderów.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-451">The wildcard character `*` is allowed, and the double wildcard `**` implies a recursive folder search.</span></span> |
| <span data-ttu-id="2e3f5-452">**buildAction**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-452">**buildAction**</span></span> | <span data-ttu-id="2e3f5-453">Akcja kompilacji, która ma zostać przypisana do elementu zawartości dla programu MSBuild, takich jak,,, `Content` `None` `Embedded Resource` `Compile` itd. Wartość domyślna to `Compile` .</span><span class="sxs-lookup"><span data-stu-id="2e3f5-453">The build action to assign to the content item for MSBuild, such as `Content`, `None`, `Embedded Resource`, `Compile`, etc. The default is `Compile`.</span></span> |
| <span data-ttu-id="2e3f5-454">**copyToOutput**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-454">**copyToOutput**</span></span> | <span data-ttu-id="2e3f5-455">Wartość logiczna wskazująca, czy elementy zawartości mają być kopiowane do folderu wyjściowego kompilacja (lub publikacja).</span><span class="sxs-lookup"><span data-stu-id="2e3f5-455">A Boolean indicating whether to copy content items to the build (or publish) output folder.</span></span> <span data-ttu-id="2e3f5-456">Wartością domyślną jest false.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-456">The default is false.</span></span> |
| <span data-ttu-id="2e3f5-457">**Flatten**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-457">**flatten**</span></span> | <span data-ttu-id="2e3f5-458">Wartość logiczna wskazująca, czy kopiować elementy zawartości do pojedynczego folderu w danych wyjściowych kompilacji (true), czy też zachować strukturę folderów w pakiecie (false).</span><span class="sxs-lookup"><span data-stu-id="2e3f5-458">A Boolean indicating whether to copy content items to a single folder in the build output (true), or to preserve the folder structure in the package (false).</span></span> <span data-ttu-id="2e3f5-459">Ta flaga działa tylko wtedy, gdy flaga copyToOutput jest ustawiona na wartość true.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-459">This flag only works when copyToOutput flag is set to true.</span></span> <span data-ttu-id="2e3f5-460">Wartością domyślną jest false.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-460">The default is false.</span></span> |

<span data-ttu-id="2e3f5-461">Podczas instalacji pakietu NuGet stosuje elementy podrzędne `<contentFiles>` od góry do dołu.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-461">When installing a package, NuGet applies the child elements of `<contentFiles>` from top to bottom.</span></span> <span data-ttu-id="2e3f5-462">Jeśli wiele wpisów pasuje do tego samego pliku, zostaną zastosowane wszystkie wpisy.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-462">If multiple entries match the same file then all entries are applied.</span></span> <span data-ttu-id="2e3f5-463">Wpis najwyższego poziomu zastępuje niższe wpisy w przypadku konfliktu dla tego samego atrybutu.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-463">The top-most entry overrides the lower entries if there is a conflict for the same attribute.</span></span>

#### <a name="package-folder-structure"></a><span data-ttu-id="2e3f5-464">Struktura folderu pakietu</span><span class="sxs-lookup"><span data-stu-id="2e3f5-464">Package folder structure</span></span>

<span data-ttu-id="2e3f5-465">Projekt pakietu powinien mieć strukturę zawartości przy użyciu następującego wzorca:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-465">The package project should structure content using the following pattern:</span></span>

```
/contentFiles/{codeLanguage}/{TxM}/{any?}
```

- <span data-ttu-id="2e3f5-466">`codeLanguages` może być `cs` ,,, `vb` `fs` `any` lub małymi literami odpowiadającymi danej `$(ProjectLanguage)`</span><span class="sxs-lookup"><span data-stu-id="2e3f5-466">`codeLanguages` may be `cs`, `vb`, `fs`, `any`, or the lowercase equivalent of a given `$(ProjectLanguage)`</span></span>
- <span data-ttu-id="2e3f5-467">`TxM` to dowolna docelowa moniker platformy docelowej, który obsługuje pakiet NuGet (patrz [Platformy docelowe](../reference/target-frameworks.md)).</span><span class="sxs-lookup"><span data-stu-id="2e3f5-467">`TxM` is any legal target framework moniker that NuGet supports (see [Target frameworks](../reference/target-frameworks.md)).</span></span>
- <span data-ttu-id="2e3f5-468">Wszystkie struktury folderów mogą być dołączane na końcu tej składni.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-468">Any folder structure may be appended to the end of this syntax.</span></span>

<span data-ttu-id="2e3f5-469">Na przykład:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-469">For example:</span></span>
...
<span data-ttu-id="2e3f5-470">Za pomocą pustych folderów można `.` zrezygnować z udostępniania zawartości dla niektórych kombinacji języka i TxM, na przykład:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-470">Empty folders can use `.` to opt out of providing content for certain combinations of language and TxM, for example:</span></span>
...
#### <a name="example-contentfiles-section"></a><span data-ttu-id="2e3f5-471">Przykładowa sekcja contentFiles</span><span class="sxs-lookup"><span data-stu-id="2e3f5-471">Example contentFiles section</span></span>
...
## <a name="framework-reference-groups"></a><span data-ttu-id="2e3f5-472">Grupy odwołań platformy</span><span class="sxs-lookup"><span data-stu-id="2e3f5-472">Framework reference groups</span></span>

<span data-ttu-id="2e3f5-473">*Tylko wersja 5.1 + wih PackageReference*</span><span class="sxs-lookup"><span data-stu-id="2e3f5-473">*Version 5.1+ wih PackageReference only*</span></span>

<span data-ttu-id="2e3f5-474">Odwołania do platformy są koncepcjami platformy .NET Core reprezentującymi współdzielone platformy, takie jak WPF lub Windows Forms.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-474">Framework References are a .NET Core concept representing shared frameworks such as WPF or Windows Forms.</span></span>
<span data-ttu-id="2e3f5-475">Określając strukturę udostępnioną, pakiet gwarantuje, że wszystkie jej zależności struktury są zawarte w projekcie odwołującym.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-475">By specifying a shared framework, the package ensures that all its framework dependencies are included in the referencing project.</span></span>

<span data-ttu-id="2e3f5-476">Każdy `<group>` element wymaga `targetFramework` atrybutu i zero lub więcej `<frameworkReference>` elementów.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-476">Each `<group>` element requires a `targetFramework` attribute and zero or more `<frameworkReference>` elements.</span></span>

<span data-ttu-id="2e3f5-477">W poniższym przykładzie przedstawiono nuspec wygenerowane dla projektu WPF platformy .NET Core.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-477">The following example shows a nuspec generated for a .NET Core WPF project.</span></span>
<span data-ttu-id="2e3f5-478">Należy zauważyć, że nie zaleca się tworzenia ręcznie nuspecs, które zawierają odwołania do struktury.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-478">Note that hand authoring nuspecs that contain framework references is not recommended.</span></span> <span data-ttu-id="2e3f5-479">Rozważ użycie pakietu [Target](msbuild-targets.md) Pack, co spowoduje automatyczne wywnioskowanie ich z projektu.</span><span class="sxs-lookup"><span data-stu-id="2e3f5-479">Consider using the [targets](msbuild-targets.md) pack instead, which will automatically infer them from the project.</span></span>
...
## <a name="example-nuspec-files"></a><span data-ttu-id="2e3f5-480">Przykładowe pliki nuspec</span><span class="sxs-lookup"><span data-stu-id="2e3f5-480">Example nuspec files</span></span>

<span data-ttu-id="2e3f5-481">**Prosta `.nuspec` , która nie określa zależności ani plików**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-481">**A simple `.nuspec` that does not specify dependencies or files**</span></span>
...
<span data-ttu-id="2e3f5-482">**A `.nuspec` z zależnościami**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-482">**A `.nuspec` with dependencies**</span></span>
...
<span data-ttu-id="2e3f5-483">**A `.nuspec` z plikami**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-483">**A `.nuspec` with files**</span></span>
...
<span data-ttu-id="2e3f5-484">**`.nuspec`Z zestawami Framework**</span><span class="sxs-lookup"><span data-stu-id="2e3f5-484">**A `.nuspec` with framework assemblies**</span></span>
...
<span data-ttu-id="2e3f5-485">W tym przykładzie są zainstalowane następujące elementy docelowe dla konkretnych projektów:</span><span class="sxs-lookup"><span data-stu-id="2e3f5-485">In this example, the following are installed for specific project targets:</span></span>

- <span data-ttu-id="2e3f5-486">. NET4 > `System.Web` , `System.Net`</span><span class="sxs-lookup"><span data-stu-id="2e3f5-486">.NET4 -> `System.Web`, `System.Net`</span></span>
- <span data-ttu-id="2e3f5-487">. Profil klienta NET4 — > `System.Net`</span><span class="sxs-lookup"><span data-stu-id="2e3f5-487">.NET4 Client Profile -> `System.Net`</span></span>
- <span data-ttu-id="2e3f5-488">Silverlight 3 — > `System.Json`</span><span class="sxs-lookup"><span data-stu-id="2e3f5-488">Silverlight 3 -> `System.Json`</span></span>
- <span data-ttu-id="2e3f5-489">WindowsPhone — > `Microsoft.Devices.Sensors`</span><span class="sxs-lookup"><span data-stu-id="2e3f5-489">WindowsPhone -> `Microsoft.Devices.Sensors`</span></span>
