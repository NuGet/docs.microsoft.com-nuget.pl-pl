...
Oprócz interfejsu użytkownika i konsoli Menedżera pakietów w programie Visual Studio, pakiet NuGet eksportuje także niektóre przydatne usługi za pośrednictwem [Managed Extensibility Framework (MEF)](/dotnet/framework/mef/index). Ten interfejs umożliwia innym składnikom programu Visual Studio współpracujące z pakietem NuGet, który może służyć do instalowania i odinstalowywania pakietów oraz do uzyskiwania informacji na temat zainstalowanych pakietów.

W ciągu lat pakiet NuGet dodał wiele usług, które znajdują się w `NuGet.VisualStudio` przestrzeni nazw `NuGet.VisualStudio.dll` zestawu:

W przypadku narzędzia NuGet 3.3 + pakiet NuGet eksportuje następujące elementy

- [`IRegistryKey`](#iregistrykey-interface): Metoda pobierania wartości z podklucza rejestru. (3.3 +)
- [`IVsCredentialProvider`](#ivscredentialprovider-interface) Zawiera metody uzyskiwania poświadczeń dla operacji NuGet. (4.0 +)
- [`IVsFrameworkCompatibility`](#ivsframeworkcompatibility-interface) Zawiera metody do odnajdywania struktur i zgodności między strukturami. (4.0 +)
- [`IVsFrameworkCompatibility2`](#ivsframeworkcompatibility2-interface) Zawiera metody do odnajdywania struktur i zgodności między strukturami. (4.0 +)
- [`IVsFrameworkCompatibility3`](#ivsframeworkcompatibility3-interface) Zawiera metody do odnajdywania struktur i zgodności między strukturami. (5.8 +)
- [`IVsFrameworkParser`](#ivsframeworkparser-interface) Interfejs do zajmowania się konwersją między ciągami a [strukturą](/dotnet/api/system.runtime.versioning.frameworkname) (4.0 +)
- [`IVsFrameworkParser2`](#ivsframeworkparser2-interface) Interfejs do analizowania ciągów .NET Framework. Zobacz [NuGet-IVsFrameworkParser](https://aka.ms/NuGet-IVsFrameworkParser). (5.8 +)
- [`IVsGlobalPackagesInitScriptExecutor`](#ivsglobalpackagesinitscriptexecutor-interface) Wykonywanie skryptów programu PowerShell z pakietów w rozwiązaniu (4.0 +)
- [`IVsNuGetFramework`](#ivsnugetframework-interface) Typ, który reprezentuje składniki monikera struktury docelowej platformy .NET. (5.8 +)
- [`IVsPackageInstaller`](#ivspackageinstaller-interface): Metody instalacji pakietów NuGet w projektach. (3.3 +)
- ["IVsPackageInstaller2](#ivspackageinstaller2-interface) Zawiera metodę instalacji najnowszej wersji pojedynczego pakietu w projekcie w ramach bieżącego rozwiązania.
- [`IVsPackageInstallerEvents`](#ivspackageinstallerevents-interface): Zdarzenia dotyczące instalowania/odinstalowywania pakietu. (3.3 +)
- [`IVsPackageInstallerProjectEvents`](#ivspackageinstallerprojectevents-interface): Zdarzenia wsadowe dotyczące instalowania/odinstalowywania pakietu. (3.3 +)
- [`IVsPackageInstallerServices`](#ivspackageinstallerservices-interface): Metody pobierania zainstalowanych pakietów w bieżącym rozwiązaniu i sprawdzania, czy dany pakiet jest zainstalowany w projekcie. (3.3 +)
- [`IVsPackageManagerProvider`](#ivspackagemanagerprovider-interface): Metody zapewniające alternatywne sugestie Menedżera pakietów dla pakietu NuGet. (3.3 +)
- [`IVsPackageMetadata`](#ivspackagemetadata-interface): Metody pobierania informacji o zainstalowanym pakiecie. (3.3 +)
- [`IVsPackageProjectMetadata`](#ivspackageprojectmetadata-interface): Metody pobierania informacji o projekcie, w którym są wykonywane akcje NuGet. (3.3 +)
- [`IVsPackageRestorer`](#ivspackagerestorer-interface): Metody przywracania pakietów zainstalowanych w projekcie. (3.3 +)
- [`IVsPackageSourceProvider`](#ivspackagesourceprovider-interface): Metody pobierania listy źródeł pakietów NuGet. (3.3 +)
- [`IVsPackageUninstaller`](#ivspackageuninstaller-interface): Metody odinstalowywania pakietów NuGet z projektów. (3.3 +)
- [`IVsPathContext`](#ivspathcontext-interface) Informacje o ścieżce NuGet specyficzne dla bieżącego kontekstu (np. kontekstu projektu). (4.0 +)
- [`IVsPathContext2`](#ivspathcontext2-interface) Informacje o ścieżce NuGet specyficzne dla bieżącego kontekstu (np. kontekstu projektu). (5.0 +)
- [`IVsPathContextProvider`](#ivspathcontextprovider-interface) Fabryka do inicjowania wystąpień [IVsPathContext](#ivspathcontext-interface) . (4.0 +)
- [`IVsPathContextProvider2`](#ivspathcontextprovider2-interface) Fabryka do inicjowania wystąpień [IVsPathContext2](#ivspathcontext2-interface) . (5.0 +)
- [`IVsProjectJsonToPackageReferenceMigrateResult`](#ivsprojectjsontopackagereferencemigrateresult-interface)  Zawiera wynik operacji migracji dla starszej project.jsw projekcie (4.3 +)
- [`IVsProjectJsonToPackageReferenceMigrator`](#ivsprojectjsontopackagereferencemigrator-interface) Zawiera metody służące do migrowania project.jsna podstawie starszego projektu do projektu opartego na PackageReference. (4.3 +)
- [`IVsSemanticVersionComparer`](#ivssemanticversioncomparer-interface) Interfejs do porównywania dwóch nieprzezroczystych ciągów wersji przez traktowanie ich jako semantyki NuGet (4.0 +)
- [`IVsTemplateWizard`](#ivstemplatewizard-interface): Zaprojektowana pod kątem szablonów projektów/elementów w celu uwzględnienia wstępnie zainstalowanych pakietów; Ten interfejs *nie* jest przeznaczony do wywoływania z kodu i nie ma metod publicznych. (3.3 +)

## <a name="using-nuget-services"></a>Korzystanie z usług NuGet

1. Zainstaluj [`NuGet.VisualStudio`](https://www.nuget.org/packages/NuGet.VisualStudio) pakiet w projekcie, który zawiera `NuGet.VisualStudio.dll` zestaw.

    Po zainstalowaniu pakiet automatycznie ustawia właściwość **Osadź typy** współdziałania dla odwołania do zestawu na **wartość true**. Dzięki temu kod odbędzie się odporny na zmiany wersji, gdy użytkownicy aktualizują się do nowszych wersji programu NuGet.

> [!Warning]
> Nie używaj żadnych innych typów oprócz interfejsów publicznych w kodzie i nie odwołują się do żadnych innych zestawów NuGet, w tym `NuGet.Core.dll` .

1. Aby użyć usługi, zaimportuj ją za pomocą [atrybutu importu MEF](/dotnet/framework/mef/index#imports-and-exports-with-attributes)lub za pomocą [usługi IComponentModel](/dotnet/api/microsoft.visualstudio.componentmodelhost.icomponentmodel).
...
W przypadku elementu Reference kod źródłowy elementu NuGet. VisualStudio jest zawarty w [repozytorium NuGet. clients](https://github.com/NuGet/NuGet.Client/tree/dev/src/NuGet.Clients/NuGet.VisualStudio).

## <a name="iregistrykey-interface"></a>IRegistryKey, interfejs
...
## <a name="ivscredentialprovider-interface"></a>IVsCredentialProvider, interfejs
...
## <a name="ivsframeworkcompatibility-interface"></a>IVsFrameworkCompatibility, interfejs
...
## <a name="ivsframeworkcompatibility2-interface"></a>IVsFrameworkCompatibility2, interfejs
...
## <a name="ivsframeworkcompatibility3-interface"></a>IVsFrameworkCompatibility3, interfejs
...
## <a name="ivsframeworkparser-interface"></a>IVsFrameworkParser, interfejs
...
## <a name="ivsframeworkparser2-interface"></a>IVsFrameworkParser2, interfejs
...
## <a name="ivsglobalpackagesinitscriptexecutor-interface"></a>IVsGlobalPackagesInitScriptExecutor, interfejs
...
## <a name="ivsnugetframework-interface"></a>IVsNuGetFramework, interfejs
...
## <a name="ivspackageinstaller-interface"></a>IVsPackageInstaller, interfejs
...
## <a name="ivspackageinstaller2-interface"></a>IVsPackageinstaller2, interfejs
...
## <a name="ivspackageinstallerevents-interface"></a>IVsPackageInstallerEvents, interfejs
...
## <a name="ivspackageinstallerprojectevents-interface"></a>IVsPackageInstallerProjectEvents, interfejs
...
## <a name="ivspackageinstallerservices-interface"></a>IVsPackageInstallerServices, interfejs
...
## <a name="ivspackagemanagerprovider-interface"></a>IVsPackageManagerProvider, interfejs
...
## <a name="ivspackagemetadata-interface"></a>IVsPackageMetadata, interfejs
...
## <a name="ivspackageprojectmetadata-interface"></a>IVsPackageProjectMetadata, interfejs
...
## <a name="ivspackagerestorer-interface"></a>IVsPackageRestorer, interfejs
...
## <a name="ivspackagesourceprovider-interface"></a>IVsPackageSourceProvider, interfejs
...
## <a name="ivspackageuninstaller-interface"></a>IVsPackageUninstaller, interfejs
...
## <a name="ivspathcontext-interface"></a>IVsPathContext, interfejs
...
## <a name="ivspathcontext2-interface"></a>IVsPathContext2, interfejs
...
## <a name="ivspathcontextprovider-interface"></a>IVsPathContextProvider, interfejs
...
## <a name="ivspathcontextprovider2-interface"></a>IVsPathContextProvider2, interfejs
...
## <a name="ivsprojectjsontopackagereferencemigrateresult-interface"></a>IVsProjectJsonToPackageReferenceMigrateResult, interfejs
...
## <a name="ivsprojectjsontopackagereferencemigrator-interface"></a>IVsProjectJsonToPackageReferenceMigrator, interfejs
...
## <a name="ivssemanticversioncomparer-interface"></a>IVsSemanticVersionComparer, interfejs
...
## <a name="ivstemplatewizard-interface"></a>IVsTemplateWizard, interfejs
