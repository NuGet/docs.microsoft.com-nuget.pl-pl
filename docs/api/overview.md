...
Aby zapoznać się z prostym wyliczeniem i pobieraniem dostępnych wersji nuget.exe, zobacz [tools.jsw](tools-json.md) punkcie końcowym.
...
[PackageBaseAddress](package-base-address-resource.md)               | yes      | Pobierz zawartość pakietu (. nupkg).
...
[PackagePublish](package-publish-resource.md)                        | yes      | Pakiety wypychania i usuwania (lub z listy).
[RegistrationsBaseUrl](registration-base-url-resource.md)            | yes      | Pobierz metadane pakietu.
...
[SearchQueryService](search-query-service-resource.md)               | yes      | Filtrowanie i wyszukiwanie pakietów według słowa kluczowego.
...
Najpierw nie dokumentuje zasobów używanych jako szczegóły implementacji NuGet.org. `SearchGalleryQueryService` Znajduje się w tej kategorii. [NuGetGallery](https://github.com/NuGet/NuGetGallery) używa tego zasobu do delegowania niektórych zapytań v2 (OData) do naszego indeksu wyszukiwania zamiast przy użyciu bazy danych. Ten zasób został wprowadzony ze względu na skalowalność i nie jest przeznaczony do użytku zewnętrznego.

Po drugie nie są udokumentowane zasoby, które nigdy nie zostały dostarczone w wersji RTM oficjalnego klienta.
`PackageDisplayMetadataUriTemplate` i należy `PackageVersionDisplayMetadataUriTemplate` do tej kategorii.

W trzecim przypadku nie dokumentuje zasobów, które są ściśle powiązane z protokołem v2, który sam jest celowo. `LegacyGallery`Zasób należy do tej kategorii. Ten zasób umożliwia indeksowi usługi v3 wskazanie odpowiedniego źródłowego adresu URL w wersji 2. Ten zasób obsługuje `nuget.exe list` .
