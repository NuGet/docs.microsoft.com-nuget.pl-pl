...
Informacje o wersji [programu NuGet DataMatrix](../release-notes/nuget-2.6.1-for-webmatrix.md)  |  [Informacje o wersji narzędzia NuGet 2.7.1](../release-notes/nuget-2.7.1.md)
...
2. [Adam pracownik2](http://www.codeplex.com/site/users/view/adamralph) ( [@adamralph](https://twitter.com/adamralph) )
...
3. [Rafael NICOLETTI](http://www.codeplex.com/site/users/view/tkrafael) ( [@tkrafael](https://twitter.com/tkrafael) )
    - Unikaj duplikowania klucza właściwości w poleceniu nuget.exe Pack.
...
6. [Tomasz Boyle](http://www.codeplex.com/site/users/view/KevinBoyleRG) ( [@kevfromireland](https://twitter.com/kevfromireland) )
...
10. [Justin Szanowny](http://www.codeplex.com/site/users/view/zippy1981) ( [@zippy1981](https://twitter.com/zippy1981) )
     - [#3307](http://nuget.codeplex.com/workitem/3307) -Naprawa usterki nie `NuGet.targets` przeszedł $ (Platform) do nuget.exe podczas przywracania pakietów.
...
W pakiecie NuGet 2,7 wprowadzono nowe podejście do przywracania pakietów, a także nastąpi główne wyróżnienie: zgoda na przywracanie pakietu jest teraz domyślnie włączona. Połączenie nowego podejścia i niejawnej zgody znacznie upraszcza scenariusze przywracania pakietów.
...
* Visual Studio 2012
* Visual Studio 2010
* Narzędzie nuget.exe Command-Line
...
#### <a name="simplified-package-restore-from-the-command-line"></a>Uproszczone przywracanie pakietu z Command-Line
...
* Witryny sieci Web systemu Windows Azure — pracują tak, aby umożliwić wypychanie projektu do platformy Azure i `nuget.exe restore` wywoływanie przed skompilowaniem witryny sieci Web.
...
Każdy z tych elementów można teraz skonfigurować w pliku znajdującym się w lokalizacji `%ProgramData%\NuGet\NuGetDefaults.Config` . Jeśli ten plik konfiguracji określa źródła pakietów, domyślne źródło pakietów nuget.org nie zostanie zarejestrowane automatycznie, a zamiast nich zostanie `NuGetDefaults.Config` zarejestrowane.

Chociaż nie jest to wymagane do korzystania z tej funkcji, oczekujemy, że firma wdraża `NuGetDefaults.Config` pliki przy użyciu zasady grupy.
...
Pakiet NuGet zawsze zarejestrował domyślne źródło pakietu o nazwie "oficjalne źródło pakietu NuGet", które wskazuje na nuget.org. Ta nazwa była pełna, a także nie została określona, gdzie faktycznie wskazywała. Aby rozwiązać te dwa problemy, zmieniono nazwę tego źródła pakietu na "nuget.org" w interfejsie użytkownika. Adres URL źródła pakietu został również zmieniony w taki sposób, aby zawierał "www". . Po użyciu programu NuGet 2,7 istniejący "oficjalne źródło pakietu NuGet" zostanie automatycznie zaktualizowany do "nuget.org" jako nazwę i " <https://www.nuget.org/api/v2/> " jako adres URL.
...
### <a name="development-only-dependencies"></a>Development-Only zależności

Ta funkcja została zaprojektowana przez program [Adam pracownik2](https://twitter.com/adamralph) i umożliwia autorom pakietów deklarowanie zależności, które były używane tylko w czasie projektowania i nie wymagają zależności pakietów. Dodanie `developmentDependency="true"` atrybutu do pakietu w programie `packages.config` `nuget.exe pack` nie spowoduje już dołączenia tego pakietu jako zależności.
