...
W tym przykładzie udostępnionego `Nuget.Config` pliku Określa folder udostępniony pakietów dla każdego projektu, który jest tworzony poniżej C:\myteam, niezależnie od głębokość. Uwaga: Jeśli masz istniejący folder pakietów poniżej katalogu głównym rozwiązanie, należy go usunąć przed NuGet spowoduje umieszczenie pakietów w nowej lokalizacji.
...
| Windows Phone (Mango) w wersji 7.5 | silverlight4 wp71 | wp71, WindowsPhone71 |
