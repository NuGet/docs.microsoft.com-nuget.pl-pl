...
A `NuGet.Config` plik jest prosty plik tekstowy XML zawierający pary klucz wartość, zgodnie z opisem w [ustawienia konfiguracji NuGet](../Schema/nuget-config-file.md) tematu.
...
- `packageSources`: Ta kolekcja ma takie samo znaczenie jak `packageSources` w regularnych config pliki i określa źródeł domyślnego w ich preferowanej kolejności. Jeśli to ustawienie istnieje w `NuGetDefaults.Config`, a następnie NuGet nuget.org nie są używane jako źródło domyślnego pakietu. Administrator w związku z tym można upewnić się, wszyscy przy użyciu tego pliku jest praca z tych samych źródeł i pozwala uniknąć przy użyciu nuget.org w razie potrzeby.
